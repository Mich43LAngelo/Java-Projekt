...
public class Pipes
{
    ...
    private static final Color COLOR = Color.blue.darker();
    ...
    public void reset()
    {
        ...
        requiredScoreForAcceleration = INIT_REQUIRED_SCORE_FOR_ACCELERATION; // po zdobyciu okreslonej liczby pkt zwieksza nam sie predkosc i przedzial
    ...
    private void addNewPair()
    {
        ...
        int bottomPipeHeight = MINIMAL_BOTTOM_PIPE_HEIGHT + randomGenerator.nextInt(boundHeight);//losowanei wysokosci

        int boundWidth = MAXIMUM_PIPE_WIDTH - MINIMAL_PIPE_WIDTH;
        int pipeWidth = MINIMAL_PIPE_WIDTH + randomGenerator.nextInt(boundWidth); //losowanie szerokosci
        ...
        bottomPipes.add(new Pipe(leftSideHorizontalCoordinate,
                                 HEIGHT - bottomPipeHeight - Ground.getGroundHeight(), pipeWidth,
                                 bottomPipeHeight));

        topPipes.add(new Pipe(leftSideHorizontalCoordinate,
                              0, pipeWidth,
                              HEIGHT - bottomPipeHeight - Ground.getGroundHeight() - GAP_BETWEEN_PIPES));// gorna z uwzglednieniem rurki
